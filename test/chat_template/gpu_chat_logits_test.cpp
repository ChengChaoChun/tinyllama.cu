...
std::string GetPrompt(int argc, char** argv) {
    if (
        argc != 3
        || std::string(argv[1]) != "--prompt"
    ) {
        throw std::runtime_error(
            "Usage: gpu-logits-test "
            "--prompt \"your prompt\""
        );
    }
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout
            << "Chat Template prompt length: " << prompt_len << " tokens\n";
        ...
        //
        // Only the last position is used for
        // the first autoregressive prediction.
        // ========================================================
        ...
        SaveNpyFloat32(
            "test/chat_template/dumps/cpp_prefill_logits.npy",
            last_prefill_logits,
            {
                1,
                static_cast<std::size_t>(
        ...
        for (int step = 0; step < max_new_tokens; ++step) {
            ...
            const std::string dump_path =
                "test/chat_template/dumps/cpp_decode_logits_"
                + std::to_string(step)
                + ".npy";
        ...
        {
            std::ofstream token_out("test/chat_template/dumps/cpp_tokens.txt");

            if (!token_out.is_open()) {
                throw std::runtime_error(
                    "Failed to create test/chat_template/dumps/cpp_tokens.txt"
                );
            }
        ...
        std::cout
            << "Logits and token IDs saved "
            << "to test/chat_template/dumps/\n";
    }
